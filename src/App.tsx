...
function App({ load, firstLoad, dataStorage }: { load: boolean, firstLoad: boolean, dataStorage: { users: User[], accounts: Account[], products: Product[] } }) {
  ...
  return (
    ...
    <main className={`${darkmode ? ' dark text-slate-300' : ' text-slate-800 '}`}>
      ...
      {preferencesOpen &&
        <div className=" bg-slate-300 dark:bg-slate-600 h-screen w-screen z-60 absolute flex items-center justify-center">
          <div className=" absolute flex flex-col items-center p-4 w-96 h-96 border dark:border-slate-600 rounded dark:bg-slate-700 bg-slate-200 shadow">
            ...
            <button onClick={() => setPreferencesOpen(false)} className=" border dark:border-red-600 dark:bg-red-500 border-red-500 rounded mt-8 bg-red-400 text-red-50 py-2 px-4 text-xl">Fechar janela de preferências</button>
          </div>
