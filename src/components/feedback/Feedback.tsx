...
export function Feedback({ isError, text, closeSelf }: { isError: boolean, text: string, closeSelf: (props:FeedbackInterface) => void }): JSX.Element {
    ...
    return (
        <>
            <div className={`${playFadeOut ? 'modal-fade-out' : 'modal-fade-in'}  overflow-hidden w-96 `}>
                <div className={`border ${closed ? 'hidden' : ' '}  ${isError ? ' border-red-500 bg-red-500 text-red-100' : 'border-emerald-500 bg-emerald-500 text-emerald-100'} shadow-inner my-2 rounded `}>
